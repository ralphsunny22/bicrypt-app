        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title">{{ __('Withdrawals') }}</h4>
                    ...
                    <table class="table table-hover custom-data-bs-table">
                        ...
                        <tbody>
                            @forelse($withdrawals as $withdraw)
                                ...
                                @endphp
                                <tr>
                                    <td data-label="{{ __('Date') }}">{{ showDateTime($withdraw->created_at) }}</td>
                                    <td data-label="@lang('Trx Number')" class="fw-bold">{{ strtoupper($withdraw->trx) }}
                                    </td>
                                    @if (!request()->routeIs('admin.users.withdrawals') && !request()->routeIs('admin.users.withdrawals.method'))
                                    ...
                                    <td data-label="{{ __('Amount') }}" class="budget fw-bold">
                                        {{ getAmount($withdraw->amount) }} {{ __($general->cur_text) }}</td>
                                    <td data-label="{{ __('Charge') }}" class="budget text-danger">
                                    ...
                                    <td data-label="{{ __('Rate') }}" class="budget">{{ getAmount($withdraw->rate) }}
                                        {{ __($withdraw->currency) }}</td>

                                    <td data-label="@lang('Payable')" class="budget fw-bold">
                                    ...
                                    @if (request()->routeIs('admin.withdraw.pending'))
                                        <td data-label="{{ __('Action') }}">
                                            <a href="{{ route('admin.withdraw.details', $withdraw->id) }}"
                                                ...
                                                data-bs-placement="top" title="{{ __('Detail') }}">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                        </td>
                                    @elseif(request()->routeIs('admin.withdraw.log') ||
                                        ...
                                        <td data-label="{{ __('Status') }}">
                                            @if ($withdraw->status == 2)
                                                <span
                                                    class="text-small badge fw-normal bg-warning">{{ __('Pending') }}</span>
                                            @elseif($withdraw->status == 1)
                                                <span
                                                    class="text-small badge fw-normal bg-success">@lang('Approved')</span>
                                    ...
                                    @if (request()->routeIs('admin.withdraw.approved') || request()->routeIs('admin.withdraw.rejected'))
                                        <td data-label="@lang('Info')">
                                            <a href="{{ route('admin.withdraw.details', $withdraw->id) }}"
                                                class="btn btn-icon btn-info ms-1 " data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="{{ __('Detail') }}">
            ...
            <div class="mb-1">{{ paginateLinks($withdrawals) }}</div>
        </div>
    </div>
        ...
        <div class="row">
            ...
            <div class="col">
                <form
                    ...
                    method="GET" class=" float-sm-right bg-white me-0 me-xl-2 me-lg-0">
                    <div class="input-group">
