                        ...
                        <tbody>
                            @forelse($methods as $method)
                                <tr>
                                    <td data-label="{{ __('method') }}">
                                        <div class="row centerize">
                                            <div class="col-md-3 thumb">
                                                <img src="{{ getImage(imagePath()['withdraw']['method']['path'] . '/' . $method->image, imagePath()['withdraw']['method']['size']) }}"
                                                    alt="{{ __('image') }}">
                                            </div>
                                    ...
                                    <td data-label="{{ __('Currency') }}" class="fw-bold">{{ __($method->currency) }}</td>
                                    <td data-label="{{ __('Charge') }}" class="fw-bold">
                                        {{ getAmount($method->fixed_charge) }} {{ __($general->cur_text) }}
                                        {{ 0 < $method->percent_charge ? ' + ' . getAmount($method->percent_charge) . ' %' : '' }}
                                    </td>
                                    <td data-label="@lang('Withdraw Limit')" class="fw-bold">{{ $method->min_limit + 0 }}
                                    ...
                                    <td data-label="{{ __('Action') }}">
                                        ...
                                        @if ($method->status == 1)
                                            <button href="javascript:void(0)"
                                                ...
                                                title="{{ __('Disable') }}" data-id="{{ $method->id }}"
                                                data-name="{{ __($method->name) }}">
                                                <i class="bi bi-eye-slash"></i>
                                            </button>
                                        @else
                                            <button href="javascript:void(0)"
                                                class="btn btn-icon btn-success btn-sm activateBtn" data-bs-toggle="tooltip"
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('admin.withdraw.method.activate') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>@lang('Are you sure to activate') <span class="fw-bold method-name"></span> {{ __('method') }}?</p>
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.withdraw.method.deactivate') }}" method="POST">
                    ...
                    <input type="hidden" name="id">
                    <div class="modal-body">
                        <p>@lang('Are you sure to disable') <span class="fw-bold method-name"></span> {{ __('method') }}?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-dark" data-bs-dismiss="modal">{{ __('Close') }}</button>
