        ...
        <div class="d-flex align-items-center w-100">
            <div class="sidebar-profile-toggle">
                <div class="avatar avatar-border">
                    <img src="{{ asset('images/portrait/small/avatar-s-11.jpg') }}" alt="user_avatar" height="42"
                        width="42" />
            ...
            @forelse ($items as $item)
                <li>
                    <span class="avatar"><img
                            src="{{ getImage(imagePath()['profileImage']['path'] . '/' . $item->user->profile_photo_path, imagePath()['profileImage']['size']) }}"
                    ...
                    <div class="chat-info flex-grow-1">
                        ...
                        <p class="card-text text-truncate">
                            {{ $item->messages[count($item->messages) - 1]['message'] }}
                        </p>
                    </div>
                    <div class="chat-meta text-nowrap">
                        <small
                            class="float-end mb-25 chat-time">{{ $item->messages[count($item->messages) - 1]['datetime'] }}</small>
                        <span class="badge bg-danger rounded-pill float-end">{{ $item->unread }}</span>
                    </div>
                </li>
            @empty
                <li class="no-results">
                    <h6 class="mb-0">No Tickets Found</h6>
                </li>
            @endforelse
        </ul>
    </div>
