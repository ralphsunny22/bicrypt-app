        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title">{{ __('Manual Deposit Methods') }}</h4>
                                ...
                                <tr>
                                    ...
                                    <td data-label="{{ __('Action') }}">
                                        <a href="{{ route('admin.payment.manual.edit', $gateway->alias) }}">
                                            <button class="btn btn-icon btn-warning btn-sm editGatewayBtn"
                                        ...
                                        @if ($gateway->status == 0)
                                            <button data-bs-toggle="modal" href="#activateModal"
                                                class="btn btn-icon btn-success btn-sm ms-1 activateBtn"
                                                data-code="{{ $gateway->code }}" data-name="{{ __($gateway->name) }}"
                                                data-original-title="{{ __('Enable') }}">
                                                <i class="bi bi-eye"></i>
                                            </button>
                                        @else
                                            <button data-bs-toggle="modal" href="#deactivateModal"
                                                class="btn btn-icon btn-danger btn-sm ms-1 deactivateBtn"
                                                data-code="{{ $gateway->code }}" data-name="{{ __($gateway->name) }}"
...
@push('breadcrumb-plugins')
    <a class="btn btn-primary" href="{{ route('admin.payment.manual.create') }}"><i
