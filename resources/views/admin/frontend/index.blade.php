...
@section('content')
        ...
        @forelse($templates as $template)
            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 col-xs-12">
                <div class="card img-cover"
                    style="@if ($template->status == 1) box-shadow: 0 4px 24px 0 rgb(7 255 0 / 27%); @endif background-image: url('{{ getImage('images/frontends/' . $template->title . '.png') }}')">
                    <div class="card-img-overlay  text-center">
                        @if ($template->activated == 1)
                            @if ($template->installed == 1)
                                <a href="{{ route('admin.template.pages', $template->id) }}"
                                    class="btn btn-icon btn-info btn-sm" style="top:80%" data-bs-toggle="tooltip"
                                ...
                                @if ($res['status'])
                                    <a type="button" class="btn btn-warning btn-sm" style="top:80%"
                                        href="{{ route('admin.template.install', $template->product_id) }}">{{ __('Update') }}
                                        Available</a>
                                @endif
                                @if ($template->status == 0)
                                    <button type="button" data-bs-toggle="modal" data-bs-target="#activateModal"
                                        ...
                                        <i class="bi bi-check-lg"></i> {{ __('Activate') }}
                                    </button>
                                @endif
                            @else
                                <a href="{{ route('admin.template.install', [$template->product_id]) }}" style="top:80%"
                                    class="btn btn-icon btn-sm btn-dark ms-1">
                                    <i class="bi bi-download"></i> {{ __('Install') }}
                                </a>
                            @endif
                        @else
                            <a href="{{ route('admin.template.activater', [$template->product_id]) }}" style="top:80%"
                                class="btn btn-icon btn-sm btn-success ms-1">
                                {{ __('Verify License') }}
                            </a>
                        @endif
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('Template Activation Confirmation') }}</h5>
    ...
    <div id="deactivateModal" class="modal fade text-start" tabindex="-1" aria-hidden="true" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('Template Disable Confirmation') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
