...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="card-title">{{ __('KYC Application') }}</h4>
        ...
        <div class="card-body">
            <div class="page-nav-wrap">
                <div class="page-nav-bar justify-content-between bg-lighter">
                    <div class="page-nav w-100 w-lg-auto">
                        <ul class="nav">
                            <li class="nav-item{{ is_page('kyc-list.pending') ? ' active' : '' }}"><a class="nav-link"
                                    href="{{ route('admin.kycs', 'pending') }}">{{ __('Pending') }}</a></li>

                            <li class="nav-item{{ is_page('kyc-list.missing') ? ' active' : '' }}"><a class="nav-link"
                                    href="{{ route('admin.kycs', 'missing') }}">{{ __('Missing') }}</a></li>

                            <li class="nav-item{{ is_page('kyc-list.approved') ? ' active' : '' }}"><a class="nav-link"
                                    href="{{ route('admin.kycs', 'approved') }}">{{ __('Approved') }}</a></li>
                            ...
                            @foreach ($kycs as $kyc)
                                <tr>
                                    ...
                                    <td data-label="Doc Front">
                                        @if ($kyc->document != null)
                                            @if (pathinfo(storage_path('app/' . $kyc->document), PATHINFO_EXTENSION) != 'pdf')
                                                <a href="{{ getImage('assets/images/kyc/' . $kyc->document) }}"
                                                    class="image-popup">{{ $kyc->documentType == 'nidcard' ? 'Front Side' : 'Document' }}</a>
                                            @else
                                                {!! $kyc->documentType == 'nidcard' ? '<a>Front Side</a>' : '<a>Document</a>' !!}
                                            @endif
                                            &nbsp; <a title="Download"
                                                href="{{ getImage('assets/images/kyc/' . $kyc->document) }}"
                                    ...
                                    <td data-label="Doc Back">
                                        @if ($kyc->document2 != null)
                                            @if (pathinfo(storage_path('app/' . $kyc->document2), PATHINFO_EXTENSION) != 'pdf')
                                                <a href="{{ getimage('assets/images/kyc/' . $kyc->document2) }}"
                                                    class="image-popup">{{ $kyc->documentType == 'nidcard' ? 'Back Side' : 'Proof' }}</a>
                                            @else
                                                {!! $kyc->documentType == 'nidcard' ? '<a>Back Side</a>' : '<a>Proof</a>' !!}
                                            @endif
                                            &nbsp; <a title="Download"
                                                href="{{ getimage('assets/images/kyc/' . $kyc->document2) }}"
                                                target="_blank"><i class="bi bi-download"></i></a>
                                        @else
                                            &nbsp;
                                        @endif
                                    ...
                                    <td data-label="Doc Proof">
                                        @if ($kyc->document3 != null)
                                            @if (pathinfo(storage_path('app/' . $kyc->document3), PATHINFO_EXTENSION) != 'pdf')
                                                <a href="{{ getimage('assets/images/kyc/' . $kyc->document3) }}"
                                                    class="image-popup">Proof</a>
                                            @else
                                                <a>Proof</a>
                                            @endif
                                            &nbsp; <a title="Download"
                                                href="{{ getimage('assets/images/kyc/' . $kyc->document3) }}"
                                                target="_blank"><i class="bi bi-download"></i></a>
                                        @else
                                            &nbsp;
                                        @endif
                                    ...
                                    <td data-label="Status">
                                        <span
                                            class="dt-status-md badge1 badge-outline badge-md badge-{{ __status($kyc->status, 'status') }}">{{ __status($kyc->status, 'text') }}</span>
                                    </td>
                                    <td data-label="Actions">
                                        <div class="dropdown">
                                            <button type="button" class="btn btn-dark btn-icon" id="dropdownMenuButton"
                                                data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                <i class="bi bi-three-dots"></i>
                                            </button>
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item"
                                                    href="{{ route('admin.kyc.view', [$kyc->id, 'kyc_details']) }}"><i
                                                        class="bi bi-eye"></i> {{ __('View Details') }}</a></a>
                                                @if ($kyc->status != 'approved')
                                                    <a class="dropdown-item kyc_action kyc_approve" data-bs-toggle="modal"
                                                        data-bs-target="#kyc_action" data-id="{{ $kyc->id }}"
                                                        data-toggle="modal" data-target="#actionkyc"><i
                                                            class="bi bi-check-square"></i> {{ __('Update Status') }}</a>
                                                    </li>
                                                @endif
                                                @if ($kyc->status == 'missing' || $kyc->status == 'rejected')
                                                    <a class="dropdown-item delete" data-bs-toggle="modal"
                                                        data-bs-target="#deleteModal" data-id="{{ $kyc->id }}"
                                                        data-name="{{ $kyc->id }}"><i class="bi bi-trash"></i>
                                                        {{ __('Delete') }}</a></li>
                                                @endif
                                            </div>
                                        </div>
            ...
            @else
                <div class="bg-light text-center rounded py-2">
                    <p><i
                            class="bi bi-arrow-down"></i><br>{{ $is_page == 'all' ? 'No KYC application found!' : 'No ' . $is_page . ' KYC application here!' }}
                    </p>
                    <p><a class="btn btn-primary btn-auto"
                            href="{{ route('admin.kycs') }}">{{ __('View All KYC Application') }}</a>
                    </p>
                </div>
            @endif
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('KYC Update Confirmation') }}</h5>
    ...
    <div id="deleteModal" class="modal fade text-start" tabindex="-1" aria-hidden="true" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('KYC Delete Confirmation') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
