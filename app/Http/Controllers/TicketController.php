...
class TicketController extends Controller
{
    ...
    {
        $ticket = SupportTicket::where('ticket', $id)->first();
        $datas = SupportMessage::with('ticket')->where('supportticket_id', $ticket->id)->latest()->get();
        $data = [];
        $unread = 0;
        for ($i = 0; $i < count($datas); $i++) {
            ...
            $unread = $datas->where('viewed_at', '<', $ticket->last_reply)->count();
        }
