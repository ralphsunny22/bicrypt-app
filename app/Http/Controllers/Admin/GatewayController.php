...
class GatewayController extends Controller
{
    ...
    {
        abort_if(Gate::denies('automated_gateways_manager_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $page_title = 'Payment Providers';
        $empty_message = 'No provider has been installed.';
        $gateways = Gateway::orderBy('name')->with('currencies')->automatic()->paginate(getPaginate(10));
        return view('admin.gateway.list', compact('page_title', 'empty_message', 'gateways'));
    }

    public function edit($alias)
    {
        abort_if(Gate::denies('automated_gateways_manager_edit'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $gateway = Gateway::automatic()->with('currencies')->where('alias', $alias)->firstOrFail();
        $page_title = 'Update Provider : ' . $gateway->name;
    ...
    public function update(Request $request, $code)
    {
        ...
        if ($request->hasFile('image')) {
            try {
                $filename = uploadImg($request->image, $path, $size, $filename);
            } catch (\Exception $exp) {
            ...
            foreach ($request->currency as $key => $currency) {
                ...
                if ($request->hasFile($uploaded_image)) {
                    try {
                        ...
                        $filename = uploadImg($request->file($uploaded_image), $path, $size);
                    } catch (\Exception $exp) {
        ...
        return redirect()->route('admin.payment.provider.edit', $gateway->alias)->withNotify($notify);
    }

    public function remove(Request $request, $code)
    {
        abort_if(Gate::denies('automated_gateways_manager_delete'), Response::HTTP_FORBIDDEN, '403 Forbidden');
