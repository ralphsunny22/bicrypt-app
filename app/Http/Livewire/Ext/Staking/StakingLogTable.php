...
class StakingLogTable extends DataTableComponent
{
    ...
    protected $model = StakingLog::class;
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Coin", "coin_id")
                ...
                ->sortable()
                ->format(
                    fn ($value, $row, Column $column) => '<a href="' . route('admin.staking.edit', $row->coin_id) . '" class="badge bg-light-danger">' . ucfirst($row->coin->title) . '</a>'
                )
                ->html(),
            ...
            Column::make("Profit", "total_profit")
                ...
                ->sortable()
                ->format(
                    fn ($value, $row, Column $column) => '<div>Yesterday Profit: <span class="fw-bold text-success">' . (getAmount($row->last_profit) ?? 0) . '</span> USDT</div><div>Total Profit: <spanclass="fw-bold text-success"> ' . (getAmount($row->total_profit) ?? 0) . ' </span> USDT</div>'
                )
                ->html(),
