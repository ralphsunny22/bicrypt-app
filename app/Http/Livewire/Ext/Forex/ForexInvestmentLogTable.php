...
class ForexInvestmentLogTable extends DataTableComponent
{
    ...
    {
        return ForexLogs::query()->where('type', 3);
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Investment Plan", "investment_id")
                ...
                ->sortable()
                ->format(
                    fn ($value, $row, Column $column) => '<a href="' . route('admin.forex.investment.edit', $row->investment_id) . '" class="badge bg-light-danger">' . ucfirst($row->investment->title) . '</a>'
                )
                ->html(),
            ...
            Column::make("Result", "result")
                ->view('extensions.admin.forex.investment_log_profit_view'),
            ...
            Column::make("Actions", "id")
                ->collapseOnMobile()
                ->format(
                    fn ($value, $row, Column $column) => "<button type='button' data-bs-toggle='modal' data-bs-target='#forexinvestmentSet' data-id='" . $row->id . "' class='set btn btn-outline-info btn-sm'>Adjust</button>"
                )
                ->html(),
        ];
    }
