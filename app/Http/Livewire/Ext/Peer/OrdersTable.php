...
{
    protected $model = P2POrder::class;
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Type", "type")
                ...
                ->sortable()
                ->format(
                    fn ($value, $row, Column $column) => '<span class="badge bg-light-primary">' . ucfirst($row->type) . '</span>'
                )
                ->html(),
            ...
            ButtonGroupColumn::make('Actions')
                ...
                ->buttons([
                    LinkColumn::make('View')
                        ->title(fn ($row) => 'View')
                        ->location(fn ($row) => route('admin.p2p.show', $row->id))
                        ->attributes(function ($row) {
                            return [
