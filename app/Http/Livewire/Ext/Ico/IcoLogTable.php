    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Token", "ico_id")
                ...
                ->sortable()
                ->format(
                    fn ($value, $row, Column $column) => '<a href="' . route('admin.ico.edit', $row->ico_id) . '" class="badge bg-light-danger">' . ucfirst($row->ico->name) . '</a>'
                )
                ->html(),
            ...
            Column::make("Status", "status")
                ...
                ->collapseOnMobile()
                ->format(
                    fn ($value, $row, Column $column) => '<span class="badge bg-light-' . ($row->status == 1 ? 'success' : ($row->status == 2 ? 'danger' : 'warning')) . '">' . ($row->status == 1 ? 'Paid' : ($row->status == 2 ? 'Canceled' : 'Pending')) . '</span>'
                )
                ->html(),
