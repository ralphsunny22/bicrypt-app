...
use App\Exports\PracticeLogExport;
...
class PracticeTable extends DataTableComponent
{

    protected $model = PracticeLog::class;
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Type", "hilow")
                ...
                ->collapseOnMobile()
                ->format(
                    fn ($value, $row, Column $column) => '<span class="badge bg-light-' . ($row->hilow != null ? 'success' : 'danger') . '">' . ($row->hilow != null ? 'High' : 'Low') . '</span>'
                )
                ->html(),
            Column::make("Result", "result")
                ...
                ->collapseOnMobile()
                ->format(
                    fn ($value, $row, Column $column) => '<span class="badge bg-light-' . ($row->result == 1 ? 'success' : ($row->result == 2 ? 'danger' : ($row->result == 3 ? 'secondary' : 'warning'))) . '">' . ($row->result == 1 ? 'Win' : ($row->result == 2 ? 'Lose' : ($row->result == 3 ? 'Draw' : 'Pending'))) . '</span>'
                )
                ->html(),
            Column::make("Status", "status")
                ...
                ->collapseOnMobile()
                ->format(
                    fn ($value, $row, Column $column) => '<span class="badge bg-light-' . ($row->status == 1 ? 'success' : 'warning') . '">' . ($row->status == 1 ? 'Completed' : 'Running') . '</span>'
                )
                ->html(),
    ...
    public function export()
    {
        ...
        $this->clearSelected();
        return Excel::download(new PracticeLogExport($logs), 'binary_practice_log.xlsx');
    }
}