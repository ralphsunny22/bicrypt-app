...
function BVstore(
    ...
    if ($cm == true) {
        ...
        if ($type == 11) {
            $total_percentage = 100 + ($plat->membership_fees ?? 1) - $plan->margin;
        } else {
            $total_percentage = 100 + ($plat->membership_fees ?? 1);
        }
        if ($cl == true) {
            if ($plat->community_line_status == 1 && $plat->community_line_clients > 0) {
                $community_line_percentage = 1 / $total_percentage;
            }
            if ($plat->community_line_clients > 0 && $plat->community_line_status == 1) {
                $clients = MLM::where('id', '!=', $mlm->id)->where('membership', '>', 0)->orderBy('community_line', 'DESC')->limit($plat->community_line_clients)->get();
                foreach ($clients as $client) {
                    BVnew(User::where('username', $client->username)->select(['id'])->first()->id, 10, $amount * $community_line_percentage);
                }
            }
        }
        ...
        switch ($type) {
            case '1':
                $commission = $plan->deposit_commission / 100;
                break;
            case '2':
                $commission = $plan->ref_commission / 100;
                break;
            case '3':
                $commission = $plan->active_ref_commission / 100;
                break;
            case '4':
                $commission = $plan->trade_commission / 100;
                break;
            case '5':
                $commission = $plan->bot_commission / 100;
                break;
            case '6':
                $commission = $plan->ico_commission / 100;
                break;
            case '7':
                $commission = $plan->forex_commission / 100;
                break;
            case '8':
                $commission = $plan->forex_investment_commission / 100;
                break;
            case '9':
                $commission = $plan->staking / 100;
                break;
            case '10':
                $commission = $community_line_percentage / 100;
                break;
            default:
                $commission = null;
                break;
        ...
        if ($plat->type == 'unilevel' && $ref->ref_by != null && $plat->unilevel_upline2_status == 1) {
            ...
            if ($daily != true) {
                BVnew($u1->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline2_percentage / ($cl == true ? $total_percentage : 100)));
                if ($u1->ref_by != null && $plat->unilevel_upline3_status == 1) {
                    $u2 = User::where('id', $u1->ref_by)->first();
                    BVnew($u2->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline3_percentage / ($cl == true ? $total_percentage : 100)));
                    if ($u2->ref_by != null && $plat->unilevel_upline4_status == 1) {
                        $u3 = User::where('id', $u2->ref_by)->first();
                        BVnew($u3->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline4_percentage / ($cl == true ? $total_percentage : 100)));
                        if ($u3->ref_by != null && $plat->unilevel_upline5_status == 1) {
                            $u4 = User::where('id', $u3->ref_by)->first();
                            BVnew($u4->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline5_percentage / ($cl == true ? $total_percentage : 100)));
                            if ($u4->ref_by != null && $plat->unilevel_upline6_status == 1) {
                                $u5 = User::where('id', $u4->ref_by)->first();
                                BVnew($u5->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline6_percentage / ($cl == true ? $total_percentage : 100)));
                                if ($u5->ref_by != null && $plat->unilevel_upline7_status == 1) {
                                    $u6 = User::where('id', $u5->ref_by)->first();
                                    BVnew($u6->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline7_percentage / ($cl == true ? $total_percentage : 100)));
                                }
                            }
                        }
                    }
                ...
                BVplan($u1->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline2_percentage / ($cl == true ? $total_percentage : 100)), $log_id, $duration);
                if ($u1->ref_by != null && $plat->unilevel_upline3_status == 1) {
                    $u2 = User::where('id', $u1->ref_by)->first();
                    BVplan($u2->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline3_percentage / ($cl == true ? $total_percentage : 100)), $log_id, $duration);
                    if ($u2->ref_by != null && $plat->unilevel_upline4_status == 1) {
                        $u3 = User::where('id', $u2->ref_by)->first();
                        BVplan($u3->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline4_percentage / ($cl == true ? $total_percentage : 100)), $log_id, $duration);
                        if ($u3->ref_by != null && $plat->unilevel_upline5_status == 1) {
                            $u4 = User::where('id', $u3->ref_by)->first();
                            BVplan($u4->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline5_percentage / ($cl == true ? $total_percentage : 100)), $log_id, $duration);
                            if ($u4->ref_by != null && $plat->unilevel_upline6_status == 1) {
                                $u5 = User::where('id', $u4->ref_by)->first();
                                BVplan($u5->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline6_percentage / ($cl == true ? $total_percentage : 100)), $log_id, $duration);
                                if ($u5->ref_by != null && $plat->unilevel_upline7_status == 1) {
                                    $u6 = User::where('id', $u5->ref_by)->first();
                                    BVplan($u6->id, $type, $amount * ($commission ?? 1) * ($plat->unilevel_upline7_percentage / ($cl == true ? $total_percentage : 100)), $log_id, $duration);
                                }
                            }
                        }
                    }
